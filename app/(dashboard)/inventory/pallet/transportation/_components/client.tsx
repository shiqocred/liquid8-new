...
export const Client = () => {
  ...
  const handleGetData = async (p?: number) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/vehicle-types?page=${p ?? page.current}&q=${searchValue}`,
        {
          headers: {
  ...
  const handleGetDetail = async (e: MouseEvent, idDetail: any) => {
    ...
    try {
      const response = await axios.get(`${baseUrl}/vehicle-types/${idDetail}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${accessToken}`,
        },
      ...
      onOpen("create-edit-transportation-palet-modal", {
        id: dataRes.id,
        name: dataRes.vehicle_name,
        length: dataRes.cargo_length,
        width: dataRes.cargo_height,
        height: dataRes.cargo_width,
      });
    } catch (err: any) {
  ...
  const handleCurrentId = useCallback(
    (q: string, p: number) => {
      ...
      const url = qs.stringifyUrl(
        {
          url: "/inventory/pallet/transportation",
          query: updateQuery,
        },
        { skipNull: true }
      );
  ...
  useEffect(() => {
    if (cookies.get("pageTransportationPalet")) {
      handleCurrentId(searchValue, page.current);
      handleGetData();
      return cookies.remove("pageTransportationPalet");
    }
  }, [cookies.get("pageTransportationPalet"), searchValue, page.current]);

  // auto update
  useEffect(() => {
    if (cookies.get("transportationPaletPage")) {
      handleGetData();
      return cookies.remove("transportationPaletPage");
    }
  }, [cookies.get("transportationPaletPage")]);
        ...
        <div className="flex flex-col w-full gap-4">
          <div className="flex gap-2 items-center w-full justify-between">
            <div className="w-full flex gap-4">
              ...
              <TooltipProviderPage value={"Reload Data"}>
                <Button
                  onClick={(e) => {
                    e.preventDefault();
                    cookies.set("transportationPaletPage", "update");
                  }}
            ...
            <Button
              ...
              type="button"
              onClick={() => onOpen("create-edit-transportation-palet-modal")}
            >
          ...
          <div className="w-full p-4 rounded-md border border-sky-400/80">
            <ScrollArea>
              <div className="flex w-full px-5 py-3 bg-sky-100 rounded text-sm gap-4 font-semibold items-center hover:bg-sky-200/80">
                ...
                <p className="w-1/3 flex-none">Name</p>
                <div className="w-full grid grid-cols-3">
                  <p className="w-full">Length</p>
                  ...
                  {data.length > 0 ? (
                    data.map((item, i) => (
                      ...
                      >
                        ...
                        <p className="w-1/3 flex-none overflow-hidden text-ellipsis whitespace-nowrap capitalize">
                          {item.vehicle_name}
                        </p>
                        <div className="w-full grid grid-cols-3">
          ...
          <Pagination
            ...
            cookie="pageTransportationPalet"
          />
          {/* <Input
            onChange={(e) => {
