...
export const Client = () => {
  ...
  const handleGetData = async (p?: number) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/product-conditions?page=${
          p ?? page.current
        }&q=${searchValue}`,
        {
          headers: {
  ...
  const handleGetDetail = async (e: MouseEvent, idDetail: any) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/product-conditions/${idDetail}`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${accessToken}`,
      ...
      onOpen("create-edit-condition-palet-modal", {
        id: dataRes.id,
        name: dataRes.condition_name,
        slug: dataRes.condition_slug,
      });
    } catch (err: any) {
  ...
  const handleCurrentId = useCallback(
    (q: string, p: number) => {
      ...
      const url = qs.stringifyUrl(
        {
          url: "/inventory/pallet/condition",
          query: updateQuery,
        },
        { skipNull: true }
      );
  ...
  useEffect(() => {
    if (cookies.get("pageConditionPalet")) {
      handleCurrentId(searchValue, page.current);
      handleGetData();
      return cookies.remove("pageConditionPalet");
    }
  }, [cookies.get("pageConditionPalet"), searchValue, page.current]);

  // auto update
  useEffect(() => {
    if (cookies.get("conditionPaletPage")) {
      handleGetData();
      return cookies.remove("conditionPaletPage");
    }
  }, [cookies.get("conditionPaletPage")]);
        ...
        <div className="flex flex-col w-full gap-4">
          <div className="flex gap-2 items-center w-full justify-between">
            <div className="w-full flex gap-4">
              ...
              <TooltipProviderPage value={"Reload Data"}>
                <Button
                  onClick={(e) => {
                    e.preventDefault();
                    cookies.set("conditionPaletPage", "update");
                  }}
            ...
            <Button
              ...
              type="button"
              onClick={() => onOpen("create-edit-condition-palet-modal")}
            >
                      ...
                      >
                        ...
                        <div className="w-52 flex flex-none  gap-4 justify-center">
                          ...
                          <Button
                            ...
                            type="button"
                            onClick={() =>
                              onOpen("delete-condition-palet-modal", item.id)
                            }
                          >
