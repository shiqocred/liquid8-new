...
export const Client = () => {
  ...
  const handleGetDetail = async (e: MouseEvent, idDetail: any) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/product-statuses/${idDetail}`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${accessToken}`,
      ...
      onOpen("create-edit-status-palet-modal", {
        id: dataRes.id,
        name: dataRes.status_name,
        slug: dataRes.status_slug,
      });
    } catch (err: any) {
  ...
  useEffect(() => {
    if (cookies.get("pageStatusPalet")) {
      handleCurrentId(searchValue, page.current);
      handleGetData();
      return cookies.remove("pageStatusPalet");
    }
  }, [cookies.get("pageStatusPalet"), searchValue, page.current]);

  // auto update
  useEffect(() => {
    if (cookies.get("statusPaletPage")) {
      handleGetData();
      return cookies.remove("statusPaletPage");
    }
  }, [cookies.get("statusPaletPage")]);
        ...
        <div className="flex flex-col w-full gap-4">
          <div className="flex gap-2 items-center w-full justify-between">
            <div className="w-full flex gap-4">
              ...
              <TooltipProviderPage value={"Reload Data"}>
                <Button
                  onClick={(e) => {
                    e.preventDefault();
                    cookies.set("statusPaletPage", "update");
                  }}
            ...
            <Button
              ...
              type="button"
              onClick={() => onOpen("create-edit-status-palet-modal")}
            >
                      ...
                      >
                        ...
                        <div className="w-52 flex flex-none  gap-4 justify-center">
                          ...
                          <Button
                            ...
                            type="button"
                            onClick={() =>
                              onOpen("delete-status-palet-modal", item.id)
                            }
                          >
