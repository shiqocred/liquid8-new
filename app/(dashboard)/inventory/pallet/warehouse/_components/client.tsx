...
export const Client = () => {
  ...
  const handleGetData = async (p?: number) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/warehouses?page=${p ?? page.current}&q=${searchValue}`,
        {
          headers: {
  ...
  const handleGetDetail = async (e: MouseEvent, idDetail: any) => {
    ...
    try {
      const response = await axios.get(`${baseUrl}/warehouses/${idDetail}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${accessToken}`,
        },
      ...
      onOpen("create-edit-warehouse-palet-modal", {
        id: dataRes.id,
        name: dataRes.nama,
        phone: dataRes.no_hp,
        address: dataRes.alamat,
        provinsi: {
          nama: dataRes.provinsi,
          slug: slug(dataRes.provinsi, "_"),
        },
        kabupaten: dataRes.kabupaten,
        kecamatan: dataRes.kecamatan,
  ...
  const handleCurrentId = useCallback(
    (q: string, p: number) => {
      ...
      const url = qs.stringifyUrl(
        {
          url: "/inventory/pallet/warehouse",
          query: updateQuery,
        },
        { skipNull: true }
      );
  ...
  useEffect(() => {
    if (cookies.get("pageWarehouse")) {
      handleCurrentId(searchValue, page.current);
      handleGetData();
      return cookies.remove("pageWarehouse");
    }
  }, [cookies.get("pageWarehouse"), searchValue, page.current]);

  // auto update
  useEffect(() => {
    if (cookies.get("warehousePage")) {
      handleGetData();
      return cookies.remove("warehousePage");
    }
  }, [cookies.get("warehousePage")]);
        ...
        <div className="flex flex-col w-full gap-4">
          <div className="flex gap-2 items-center w-full justify-between">
            <div className="w-full flex gap-4">
              ...
              <TooltipProviderPage value={"Reload Data"}>
                <Button
                  onClick={(e) => {
                    e.preventDefault();
                    cookies.set("warehousePage", "update");
                  }}
            ...
            <Button
              ...
              type="button"
              onClick={() => onOpen("create-edit-warehouse-palet-modal")}
            >
          ...
          <div className="w-full p-4 rounded-md border border-sky-400/80">
            <ScrollArea>
              <div className="flex w-full px-5 py-3 bg-sky-100 rounded text-sm gap-4 font-semibold items-center hover:bg-sky-200/80">
                ...
                <p className="min-w-32 w-full max-w-52">Name</p>
                <p className="w-36 flex-none">Phone Number</p>
                <p className="w-full min-w-44 max-w-[350px]">Address</p>
                <p className="min-w-20 max-w-52 w-full ">Kec.</p>
                <p className="min-w-20 max-w-52 w-full ">Kab./Kota</p>
                <p className="min-w-20 max-w-52 w-full ">Prov.</p>
                <p className="min-w-20 max-w-52 w-full ">Lat.</p>
                <p className="min-w-20 max-w-52 w-full ">Long.</p>
                ...
                <div className="w-full min-h-[200px]">
                  {Array.from({ length: 15 }, (_, i) => (
                    ...
                    >
                      ...
                      <div className="min-w-32 w-full max-w-52 ">
                        <Skeleton className="w-52 h-4" />
                      </div>
                      <div className="w-36 flex-none ">
                        <Skeleton className="w-28 h-4" />
                      </div>
                      <div className="w-full min-w-44 max-w-[350px]">
                        <Skeleton className="w-44 h-4" />
                      </div>
                      <div className="min-w-20 max-w-52 w-full  ">
                        <Skeleton className="w-44 h-4" />
                      </div>
                      <div className="min-w-20 max-w-52 w-full  ">
                        <Skeleton className="w-44 h-4" />
                  ...
                  {data.length > 0 ? (
                    data.map((item, i) => (
                      ...
                      >
                        ...
                        <p className="min-w-32 w-full max-w-52 overflow-hidden text-ellipsis whitespace-nowrap capitalize">
                          {item.nama}
                        </p>
                        <p className="w-36 flex-none overflow-hidden text-ellipsis whitespace-nowrap">
                          {item.no_hp}
                        </p>
                        <p className="w-full min-w-44 max-w-[350px] overflow-hidden text-ellipsis whitespace-nowrap capitalize">
                          {item.alamat}
                        </p>
                        <p className="min-w-20 max-w-52 w-full  overflow-hidden text-ellipsis whitespace-nowrap capitalize">
                          {item.kecamatan}
                        </p>
                        <p className="min-w-20 max-w-52 w-full  overflow-hidden text-ellipsis whitespace-nowrap capitalize">
                          {item.kabupaten}
                        ...
                        <div className="w-52 flex flex-none  gap-4 justify-center">
                          ...
                          <Button
                            ...
                            type="button"
                            onClick={() =>
                              onOpen("delete-warehouse-palet-modal", item.id)
                            }
                          >
