...
export const Client = () => {
  ...
  const handleGetData = async (p?: number) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/migrate-bulky?page=${p ?? page.current}&q=${searchValue}`,
        {
          headers: {
  ...
  const handleGetDetail = async (e: MouseEvent, idDetail: any) => {
    ...
    try {
      const response = await axios.get(`${baseUrl}/migrate-bulky/${idDetail}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${accessToken}`,
        },
          ...
          <div className="w-full p-4 rounded-md border border-sky-400/80">
            <ScrollArea>
              <div className="flex w-full px-5 py-3 bg-sky-100 rounded text-sm gap-4 font-semibold items-center hover:bg-sky-200/80">
                ...
                <p className="w-full min-w-72 max-w-[500px]">Name User</p>
                <p className="w-52 flex-none">Document Code</p>
                <p className="w-40 flex-none">Status</p>
                <p className="w-32 text-center flex-none ml-auto">Action</p>
              </div>
              {loading ? (
                <div className="w-full">
                  {Array.from({ length: 15 }, (_, i) => (
                    ...
                    >
                      ...
                      <div className="w-32 flex-none flex gap-4 justify-center ml-auto">
                  ...
                  {data.length > 0 ? (
                    data.map((item, i) => (
                      ...
                      >
                        ...
                        <p className="w-40 flex-none whitespace-nowrap text-ellipsis overflow-hidden">
