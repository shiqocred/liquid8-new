...
export const Client = () => {
  ...
  const handleGetData = async (p?: number) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/destinations?page=${p ?? page.current}&q=${searchValue}`,
        {
          headers: {
  ...
  const handleGetDetail = async (e: MouseEvent, idDetail: any) => {
    ...
    try {
      const response = await axios.get(`${baseUrl}/destinations/${idDetail}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${accessToken}`,
        },
      ...
      onOpen("create-edit-destination-modal", {
        name: dataRes?.shop_name,
        address: dataRes?.alamat,
        phone: dataRes?.phone_number,
        id: dataRes?.id,
      });
    } catch (err: any) {
  ...
  const handleCurrentId = useCallback(
    (q: string, p: number) => {
      ...
      const url = qs.stringifyUrl(
        {
          url: "/outbond/migrate-color/destination",
          query: updateQuery,
        },
        { skipNull: true }
      );
  ...
  useEffect(() => {
    if (cookies.get("pageDestination")) {
      handleCurrentId(searchValue, page.current);
      handleGetData();
      return cookies.remove("pageDestination");
    }
  }, [cookies.get("pageDestination"), searchValue, page.current]);

  // auto update
  useEffect(() => {
    if (cookies.get("destinationPage")) {
      handleGetData();
      return cookies.remove("destinationPage");
    }
  }, [cookies.get("destinationPage")]);
        ...
        <div className="flex flex-col w-full gap-4">
          <div className="flex gap-2 items-center w-full justify-between">
            <div className="w-full flex gap-4">
              ...
              <TooltipProviderPage value={"Reload Data"}>
                <Button
                  onClick={(e) => {
                    e.preventDefault();
                    cookies.set("destinationPage", "update");
                  }}
            ...
            <Button
              ...
              onClick={() => onOpen("create-edit-destination-modal")}
              type="button"
            >
                  ...
                  {data.length > 0 ? (
                    data.map((item, i) => (
                      ...
                      >
                        ...
                        <p className="w-72 flex-none overflow-hidden text-ellipsis whitespace-nowrap capitalize">
                          {item.shop_name}
                        </p>
                        ...
                        <div className="w-52 flex-none flex gap-4 justify-center">
                          ...
                          <Button
                            ...
                            type="button"
                            onClick={() =>
                              onOpen("delete-destination-modal", item.id)
                            }
                          >
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    cookies.set("pageDestination", "removed");
                  }}
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    cookies.set("pageDestination", "added");
                  }}
