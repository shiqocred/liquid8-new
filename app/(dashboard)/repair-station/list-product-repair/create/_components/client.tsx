...
const Client = () => {
  ...
  const handleGetDataFiltered = async () => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}/repair-mv/filter_product?page=${page.current}`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${accessToken}`,
  ...
  const handleAddProduct = async (e: MouseEvent, id: number) => {
    ...
    try {
      ...
      cookies.set("updateProductLPRFiltered", "removed");
      cookies.set("updateProductLPR", "updated");
    } catch (err: any) {
  ...
  const handleDeleteProduct = async (e: MouseEvent, id: number) => {
    ...
    try {
      ...
      cookies.set("updateProductLPRFiltered", "removed");
      cookies.set("updateProductLPR", "removed");
    } catch (err: any) {
    ...
    try {
      ...
      toast.success(
        response.data.data.message ??
          "List product repair successfully created."
      );
      router.push("/inventory/moving-product/repair");
    } catch (err: any) {
      toast.error(
        err.response.data.data.message ??
          "List product repair failed to create."
      );
  ...
  useEffect(() => {
    if (cookies.get("LPRProduct")) {
      handleGetData();
      return cookies.remove("LPRProduct");
    }
  }, [cookies.get("LPRProduct"), page.current]);
  useEffect(() => {
  ...
  // auto update
  useEffect(() => {
    if (cookies.get("updateProductLPRFiltered")) {
      handleGetDataFiltered();
      return cookies.remove("updateProductLPRFiltered");
    }
  }, [cookies.get("updateProductLPRFiltered")]);
  useEffect(() => {
    if (cookies.get("updateProductLPR")) {
      handleGetData();
      return cookies.remove("updateProductLPR");
    }
  }, [cookies.get("updateProductLPR")]);
  ...
  return (
    <div className="flex flex-col items-start bg-gray-100 w-full relative px-4 gap-4 py-4">
      ...
      <div className="flex w-full bg-white rounded-md overflow-hidden shadow px-5 py-3 flex-col gap-3">
        <div className="w-full my-3 text-xl font-semibold flex justify-between items-center gap-4">
          ...
          <div className="flex gap-4">
            <TooltipProviderPage value={"Reload Data"}>
              <Button
                onClick={(e) => {
                  e.preventDefault();
                  cookies.set("updateProductLPRFiltered", "update");
                }}
                  ...
                  <div className="flex gap-4 items-center w-full">
                    ...
                    <TooltipProviderPage value={"Reload Data"}>
                      <Button
                        onClick={(e) => {
                          e.preventDefault();
                          cookies.set("updateProductLPR", "update");
                        }}
                        className="items-center w-9 px-0 flex-none h-9 border-sky-400 text-black hover:bg-sky-50"
                        variant={"outline"}
                      >
